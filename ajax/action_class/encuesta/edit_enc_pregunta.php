...
if($row['habilitar_comentario'] == 0 && $tipo_pregunta == 1){
    ...
    if($tipo_pregunta == 1){
        if($result = $object->edit_enc_pregunta_comentario($txt_pregunta, $tipo_pregunta, $date_actualizacion, $estado_pregunta, $id_encuesta, $id_enc_pregunta)){
            echo "true";
        }else{
            echo "false";
        }
    }else{
        if($result = $object->edit_enc_pregunta($txt_pregunta, $tipo_pregunta, $num_opciones, $date_actualizacion, $estado_pregunta, $id_encuesta, $id_enc_pregunta)){
            echo "true";
        }else{
            echo "false";
        }
