...
<?php
    ...
    $_SESSION['filemanager_categoria'] = "matriz_jobs";
    ...
    if($row_permisos['add_ma_jobs'] != "true"){
?> 
<div class="modal-content">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
...
<?php }else{ ?>
        <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal">&times;</button>
              <h3 class="modal-title">Agregar Matriz de Jobs</h3>
            </div>
            <div class="modal-body">
                <form enctype="multipart/form-data" id="insert_jobs" class="sky-form">
                    <fieldset>
                        <div class="row">
                            <section class="col col-10">
                                <label>Ubicacion de archivo de Jobs (.csv)</label>
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-file-excel-o"></i></span>
                                    <input name="sel_file_jobs" type="text" class="sel_file_jobs form-control" id="inputNas_JOBS" placeholder="Ruta del archivo a Subir">
                                </div>
                            </section>
                            <section class="col col-2">
                                <label>&nbsp;</label>
                                <div class="input-group">
                                    <a class="btn btn-info" style="color:white" onclick="javascript:ventana_url('assets/js/tinymce/filemanager/dialog.php?popup=1&amp;field_id=inputNas_JOBS')"><i class="fa fa-upload"></i> Examinar</a>
                                </div>
                            </section>
                        </div>
...
<script>
    ...
    function abrir_loading(){
        swal({
            title: "",
            text: "Cargando la matriz de Jobs.....",
            type: "info",
    ...
    $('#insert_jobs').submit(function( event ) {
        if ($(".sel_file_jobs").val() == '') {
            swal("PID - CLARO AN", "Favor de completar los campos", "warning");
            return false;
            }else{
                var formData = $(this).serialize();
                swal({
                    title: 'Deseas agregar esta base de Jobs ?',
                    ...
                    $.ajax({
                        type: "POST",
                        url: "ajax/action_class/insert/insert_jobs.php",
                        data: formData,
                        success: function(data) {
                            ...
                            if(data == 0){
                                ...
                                setTimeout(function(){
                                    swal({
                                        title: "",
                                        text: "Se agrego los datos de JOBS con exito",
                                        type: "success",
                                        showCancelButton: false,
                                        showConfirmButton: true
                                    }).then(function() {
