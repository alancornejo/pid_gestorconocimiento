	...
	$.fn.paginate = function( options ) {
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				$( "." + options.pagination_class ).append( '<li class="prev"><a style="cursor: pointer">' + options.prev_text + '</a></li>' );
			}
			// For each div (slide) add a link in span for controls
			for ( var i = 0; i < pages; i++ ) {
				$( "." + options.pagination_class ).append( '<li><a style="cursor: pointer">' + ( i + 1 ) + '</a></li>' );
			};

			// Add Next Button
			if ( options.prev_next == true ) {
				$( "." + options.pagination_class ).append( '<li class="next"><a style="cursor: pointer">' + options.next_text + '</a></li>' );
			}
		}
